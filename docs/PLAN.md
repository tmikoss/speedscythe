...
- **General:**
  - "Shortcuts" section: "Open picker", "Stop timer in the panel", "Edit notes in the panel", and "Start the last task in the panel" (recorders for `.openPicker`, `.stopTimer`, `.editNotes`, and `.startLastTask`), and a "Reset shortcuts" button. The button resets all four shortcuts to their defaults. "Open picker" uses `KeyboardShortcuts.Recorder`, which accepts only shortcuts with a modifier, because the shortcut is global. The three panel shortcuts use `PanelShortcutRecorder`, a copy of the library recorder that looks the same and also accepts keys without a modifier. It refuses bare digits with a beep, because the board uses them. In both recorders, ⌫ clears the shortcut and Esc ends the recording. The button is the only way back to the plain `⌫` default.
